...
class Main extends PluginBase implements Listener{
    ...
    public function loadTiers(): void{
        ...
        $this->getLogger()->notice("Tải lại tất cả cấp độ !");
    }

    public function getTier(int $tier): array{
        if(!isset($this->tiers[$tier])){
            $this->getLogger()->error("Cấp độ $tier không tồn tại !");
            return [];
        }
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool{
        if($command->getName() === "rpoint"){
            if(!$sender->isOp()){
                $sender->sendMessage(TextFormat::RED . "It does not seem like you can run that command");
                return false;
            }
            if(!isset($args[0])){
                $sender->sendMessage("Usage: /rpoint <Cấp độ> <Người chơi>");
                return false;
            }
            if(!is_numeric($args[0])){
                $sender->sendMessage(TextFormat::RED . "Xin hãy chọn 1 cấp độ thích hợp !");
                return false;
            }
            if(!isset($args[1])){
                if(!$sender instanceof Player){
                    $sender->sendMessage(TextFormat::RED . "Bạn không thể lấy cho bạn rương quay point !");
                    return false;
                }
                $this->givePointPouch($sender, $sender, $args);
                return false;
            }

            if(!$player = $this->getServer()->getPlayer($args[1])){
                $sender->sendMessage(TextFormat::RED . "Người chơi không tồn tại !");
                return false;
            }
            $this->givePointPouch($sender, $player, $args);
            return true;
        }
        return true;
    }

    public function givePointPouch(CommandSender $sender, Player $player, array $args): void{
        if(!isset($this->tiers[$args[0]])){
            $sender->sendMessage(TextFormat::RED . "Cấp độ đó không tồn tại !");
            return;
        }
    ...
    public function translateWon(string $message, int $amount): string{
        $message = str_replace("{amount}", "Point" . $amount, $message);
        return $message;
    }
    ...
    public function choose(Player $player, Item $item, Block $block): void{
        ...
        PointAPI::getInstance()->addPoint($player, $rand);
